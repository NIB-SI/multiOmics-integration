...
# no additional normalisation
data_norm2 = data_filt
data_norm2@assays@data[[1]] = as.data.frame(preprocessCore::normalize.quantiles(as.matrix(data_norm2@assays@data[[1]])))
plot_normalization(data_norm2)

data_norm3 = data_filt
data_norm3@assays@data[[1]] = as.data.frame(limma::normalizeVSN(as.matrix(data_norm3@assays@data[[1]])))
plot_normalization(data_norm3)

data_norm = data_filt
limma::plotDensities(data_norm@assays@data[[1]], main = 'Filtered, log2"', legend = FALSE)
limma::plotDensities(data_norm2@assays@data[[1]], main = 'Quantile', legend = FALSE)
limma::plotDensities(data_norm3@assays@data[[1]], main = 'VSN', legend = FALSE)
...
dep = add_rejections(data_diff_manual, alpha = threshold)# lfc = log2(1.5))
...
for (i in setdiff(myContrasts, c("D_S8_vs_C_S8", "D_S14_vs_C_S14" ))) {
  ...
  p = plot_volcano(dep, contrast = contrast, label_size = 2, add_names = FALSE, adjusted = FALSE) 
  plots[[cnt]] = p
  cnt = cnt + 1
}
