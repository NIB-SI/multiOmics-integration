mycim <-
function (mat = NULL, color = NULL, row.names = TRUE, col.names = TRUE, 
    row.sideColors = NULL, col.sideColors = NULL, row.cex = NULL, 
    col.cex = NULL, cutoff = 0, cluster = "both", dist.method = c("euclidean", 
    ...
    legend = NULL, save = NULL, name.save = NULL) 
{
    ...
    if (any(class.object %in% object.list)) {
        ...
        if (any(class.object %in% object.pca)) {
            ...
            if (any(class.object %in% c("mixo_splsda", "mixo_plsda", 
                ...
                if (any(class.object %in% c("mixo_splsda", "mixo_mlsplsda"))) 
                  keep.X = apply(abs(mat$loadings$X[, comp, drop = FALSE]), 
                    1, sum) > 0
                else keep.X = apply(abs(mat$loadings$X), 1, sum) > 
                  0
                ...
                if (any(class.object %in% c("spca", "sipca"))) 
                  keep.X = apply(abs(mat$rotation[, comp]), 1, 
                    sum) > 0
            ...
            if ((cluster == "both") || (cluster == "row")) {
                ...
                hcr = myhclust(dist.mat, method = clust.method[1])
            ...
            if ((cluster == "both") || (cluster == "column")) {
                ...
                hcc = myhclust(dist.mat, method = clust.method[2])
            ...
            if (mat$mode == "canonical") {
                bisect = mat$variates$X[, comp] + mat$variates$Y[, 
                  comp]
                cord.X = cor(mat$X[, keep.X, drop = FALSE], bisect, 
                  use = "pairwise")
                cord.Y = cor(mat$Y[, keep.Y, drop = FALSE], bisect, 
                  use = "pairwise")
            }
            else {
                cord.X = cor(mat$X[, keep.X, drop = FALSE], mat$variates$X[, 
                  comp], use = "pairwise")
                cord.Y = cor(mat$Y[, keep.Y, drop = FALSE], mat$variates$X[, 
                  comp], use = "pairwise")
            }
            XY.mat = as.matrix(cord.X %*% t(cord.Y))
            sample.sideColors = row.sideColors
    ...
    try_plot <- tryCatch({
        mixOmics:::imageMap(object, color = color, row.names = row.names, 
    ...
    if (is(try_plot, "error")) {
        ...
        if (!is.null(legend)) {
            ...
            if (is.null(legend$cex)) 
                legend$cex = 0.80000000000000004
            if (any(class.object %in% c("mixo_splsda", "mixo_plsda"))) {
...
myhclust <- 
  function (d, method = "complete", members = NULL) 
  {
    ...
    write.table(as.matrix(d),file="distance.txt", col.names=NA,sep="\t")
