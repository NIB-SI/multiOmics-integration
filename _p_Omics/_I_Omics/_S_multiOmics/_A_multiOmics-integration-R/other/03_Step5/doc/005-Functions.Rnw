...
Panel functions for function \fct{pairs}
<<>>=
...
@
\clearpage
\subsection{\fct{plt.var}}
Adapted function \fct{plt.var}. For \code{X}, correlation between \code{X} and scores of \code{X} is plotted (namely coefficients of \code{X}). I assume that it should also be for \code{Y} instead of correlation between \code{Y} and scores of \code{X}.
<<>>=
