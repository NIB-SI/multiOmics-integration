...
%%#plotVar(res, cutoff=0.5, legend = TRUE, overlap=!FALSE, style='graphics')
%%#plotVar(res, cutoff=0.5, legend = TRUE, overlap=FALSE, style='graphics')
%%plotVar(res, cutoff=0.5, legend = FALSE, comp=c(1,2), overlap=FALSE, style='ggplot2', col=cols)
%%plotVar(res, cutoff=0.5, legend = FALSE, comp=c(2,3), overlap=FALSE, col=cols)
%%```
...
```{r}
netfn <- paste0("network-",paste(.treat, collapse="_"),"-",paste(datasets, collapse="_"),".txt")
...
write(file.path(suffix,netfn), "bla.log", append=TRUE)
write(netfn, "bla.log", append=TRUE)
