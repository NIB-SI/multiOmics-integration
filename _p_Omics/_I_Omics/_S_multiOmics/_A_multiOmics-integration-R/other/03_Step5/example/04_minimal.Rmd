...
It is advisable to first read the phenodata (or sample description file), followed by actual data input. This enables smart selection of samples, based on the sample selection column with the assay name.
...
```{r}
hormonomics <- read.table(file.path(.aroot,"data_hormonomics.txt"), header=TRUE, sep="\t")
metabolomics <- read.table(file.path(.aroot,"data_metabolomics.txt"), header=TRUE, sep="\t")
qPCR <- read.table(file.path(.aroot,"data_qPCR.txt"), header=TRUE, sep="\t")
```
