...
One idea is to randomize treatments by permuting the combined status codes before attributing them to the actual sample conditions.
...
if(randomize) {
set.seed(1234)
  what <- sample(what, length(what), FALSE)
  pdata$Treatment <- substr(what,1,1)
  pdata$SamplingDay <- substr(what,2,3)
  print(pdata$Treatment)
  print(pdata$SamplingDay)
}
what
X <- list(status= what)
names(X[[1]]) <- rownames(pdata)
...
if(randomize){
print(addmargins(table(sday, what)), zero.print=".")
print(addmargins(table(trt, what)), zero.print=".")
}
```
