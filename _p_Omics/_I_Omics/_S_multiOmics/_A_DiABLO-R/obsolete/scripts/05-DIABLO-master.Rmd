...
In this document we present a workflow for integration across different omics datasets.

Moreover, suing sparse Partial Least Squares Discriminant Analysis (DIABLO, multiblock PLS-DA) approach for horizontal (N-Integration) and canonical-correlation analysis (CCA) we will generate (pairwise) inputs for differential network construction using automatic thresholding.


[Note] This is not the final version of the document. Network figure generation may cause some unexpected issues. For more help visit <https://mixomics-users.discourse.group/>
...
Read more about packages at <https://mixomics.org/> and <https://r.igraph.org/>.
...
Short names for proteomics subset

```{r}

pretty = as.data.frame(data.table::fread(file.path('..', 'input', 'Proteomics-subset-annotation.txt'))[, 1:2])
ind = match(colnames(DATA$proteomics), pretty[ , 1])
colnames(DATA$proteomics) = pretty[ind, 2]
