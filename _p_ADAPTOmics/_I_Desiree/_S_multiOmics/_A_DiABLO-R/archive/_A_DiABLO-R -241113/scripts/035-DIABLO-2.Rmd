...
```{r}
modfn <- paste0(oroot,"/model-",paste(.treat, collapse="_"),"-",paste(names(CCDATA[-1]), collapse="_"),".R")
dump("final.diablo.model", file=modfn)
```
...
```{r}
cimfn <- paste0(opts_chunk$get("fig.path"),"cim.png")
```
...
#
nname <- paste(opts_chunk$get("fig.path"),"network-",paste(useTreatment,collapse=""),sep="")
N12 <- my.network(res
    , cutoff = 0
    , blocks = 1:ndatasets
...
```{r}
netfn <- paste0(oroot,"/network-",paste(.treat, collapse="_"),"-",paste(datasets, collapse="_"),".txt")
