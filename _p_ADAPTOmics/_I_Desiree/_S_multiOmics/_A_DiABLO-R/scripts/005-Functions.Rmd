...
Check the pairs of columns that produce NAs in calculations

```{r}
find_NA_pairs <- function(x, verbose=FALSE){
    XX <- x
     a <- expand.grid(1:ncol(XX),1:ncol(XX))
    nas <- apply(a, 1, function(x) all(is.na(unlist(XX[,x[1]]+XX[,x[2]]))))
    bad <- unique(unlist((a[nas,1])))
        if(length(bad)>0){ 
          res <- dimnames(XX)[[2]][bad]
    if(verbose ) {
