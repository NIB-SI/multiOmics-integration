...
my.pheatmap.logFC = function(df, forHeatmap, cluster_rows = cluster_rows, cluster_cols = cluster_cols,
  ...
  logFC = df[, grep('molecule$|stressGroup$|controlGroup$|log2FC', colnames(df))]
  logFC$comparison = paste(logFC$stressGroup, logFC$controlGroup, sep = '-')
  logFC = logFC[, -c(2,3)]
  
  p = df[, grep('molecule$|stressGroup$|controlGroup$|^p$', colnames(df))]
  ...
  ind = match(sort(match(rownames(logFC.w), path$name)), match(rownames(logFC.w), path$name))
  logFC.w = logFC.w[ind, ]
  ind = match(sort(match(colnames(logFC.w), forHeatmap$comparison)), match(colnames(logFC.w), forHeatmap$comparison))
  logFC.w = logFC.w[, ind]
  
  ind = match(sort(match(rownames(p.w), path$name)), match(rownames(p.w), path$name))
  p.w = p.w[ind, ]
  ind = match(sort(match(colnames(p.w), forHeatmap$comparison)), match(colnames(p.w), forHeatmap$comparison))
  p.w = p.w[, ind]
  ...
  # row_colors = cc[match(rownames(logFC.w), cc$display_name), ]$color_code
...
fpi = file.path('..', 'other')
fn = 'path.txt'
path = as.data.frame(data.table::fread(file.path(fpi, fn)))
path = path[order(path$pathway), ]

fpi = file.path('..', 'other')
fn = 'colourCodes_v2.txt'
cc = as.data.frame(data.table::fread(file.path(fpi, fn)))
...
which(colnames(omics)[-1] %nin% cc$display_name)
...
forHeatmap[, 1] = sapply(forHeatmap[, 1], function(x) paste0(gsub("_(\\d+)", '', x), sprintf("_%02d", as.numeric(gsub(".*_", "", x)))))
forHeatmap[, 3] = sapply(forHeatmap[, 3], function(x) paste0(gsub("_(\\d+)", '', x), sprintf("_%02d", as.numeric(gsub(".*_", "", x)))))
forHeatmap$comparison = paste(forHeatmap[, 1], forHeatmap[, 3], sep = '-')
forHeatmap = forHeatmap[match(forHeatmap[, 1], c("H_01", "H_07","H_08","H_14",
                                                 "HD_08", "HD_14",
                                                 "D_08","D_14",
                                                 "W_01", "W_07")), ]

Desiree.stat$controlGroup = sapply(Desiree.stat$controlGroup, function(x) paste0(gsub("_(\\d+)", '', x), sprintf("_%02d", as.numeric(gsub(".*_", "", x)))))
...
which(colnames(omics)[-1] %nin% cc$display_name)
colnames(omics)[grep('RD29B', colnames(omics))] = 'RD29'
colnames(omics)[grep('M0ZJG3', colnames(omics))] = 'MES'
...
which(colnames(omics)[-c(1:4)] %nin% cc$display_name)
